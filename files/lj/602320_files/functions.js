...
function showParent(link){
	...
	var thisId = $(link).getParent('.post').id;
	var parent = $(parentId); 
	if(parent){
		addClass(parent, 'show');
		if (!parent.getElement('.show_previous') || parent.getElement('.show_previous').href != '#' + thisId) {
			if (parent.getElement('.show_previous')) {
				parent.getElement('.show_previous').destroy();
			}
			...
			prevLink.inject(parent.getElement('.dd .p'));
		}
